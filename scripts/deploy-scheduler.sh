...
check_permissions() {
    if [[ $EUID -eq 0 ]]; then
        error "This script should not be run as root. Use your regular user account."
        exit 1
    fi
...
deploy_files() {
    ...
    sudo cp "$BUILD_DIR/$BINARY_NAME" "$DEPLOY_DIR/"
    sudo chmod +x "$DEPLOY_DIR/$BINARY_NAME"
    
    # Deploy templates
    log "Deploying templates..."
    sudo cp -r "$PROJECT_ROOT/src/templates/"* "$DEPLOY_DIR/templates/"
    
    # Deploy static assets
    log "Deploying static assets..."
    if [[ -d "$PROJECT_ROOT/images" ]]; then
        sudo cp -r "$PROJECT_ROOT/images/"* "$DEPLOY_DIR/images/" 2>/dev/null || true
    fi
    
    # Deploy configuration files
    log "Deploying configuration..."
    if [[ -f "$PROJECT_ROOT/.env" ]]; then
        sudo cp "$PROJECT_ROOT/.env" "$DEPLOY_DIR/"
    ...
    if [[ -d "$PROJECT_ROOT/certs" ]]; then
        ...
        sudo cp -r "$PROJECT_ROOT/certs/"* "$DEPLOY_DIR/certs/" 2>/dev/null || true
    fi
