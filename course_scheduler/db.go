...
func (scheduler *wmu_scheduler) AddUser(username, password string) error {
	...
	_, err = scheduler.database.Exec("INSERT INTO users (username, password) VALUES (?, ?)", username, hashed)
	return err
}
...
func (scheduler *wmu_scheduler) AuthenticateUser(username, password string) (bool, error) {
	if !ValidateEmail(username) {
		return false, errors.New("invalid email address")
	}
	var hashedPassword string
	err := scheduler.database.QueryRow("SELECT password FROM users WHERE username = ?", username).Scan(&hashedPassword)
...
func (scheduler *wmu_scheduler) GetUserLoggedInStatus(username string) (bool, error) {
	if !ValidateEmail(username) {
		return false, errors.New("invalid email address")
	}
	var isLoggedIn bool
	err := scheduler.database.QueryRow("SELECT is_logged_in FROM users WHERE username = ?", username).Scan(&isLoggedIn)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
